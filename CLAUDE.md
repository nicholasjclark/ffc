...
The `ffc` (Functional ForeCasting) package performs functional
regression using GAMs with time-varying functional coefficients,
integrating mgcv and Stan for dynamic factor modeling.
...
This package uses the S3 object system with classes: - `ffc_gam` - Main
model class - `fts_ts` - Time series coefficient class - Methods:
`print`, `predict`, `forecast`, `autoplot`, `model.frame`
...
- **ALL exported functions must be assigned to a reference section** in
  `_pkgdown.yml`
...
- If function doesn’t fit existing sections, create new appropriate
  section
...
- Add vignettes to articles:contents using either:
  - Individual vignette name without .Rmd extension (e.g.,
    `elnino-forecasting`)
...
- NO [`try()`](https://rdrr.io/r/base/try.html),
  [`tryCatch()`](https://rdrr.io/r/base/conditions.html), or `skip()` in
  tests
