...
`fts()` term, which decomposes functional time series into time-varying
basis coefficients that can be forecasted using either independent time
series models from the `fable` package or with efficient dynamic factor
models using precompiled Stan models.
...
The `autoplot()` method is handy for viewing the time-varying basis
coefficients from `ffc_gam()` models. Here we draw 10 realisations from
the estimated coefficient distributions and plot them.

    fts_coefs(mod, times = 10, summary = FALSE) |>
      autoplot()

<img src="man/figures/README-tourism-coefficients0-1.png" alt="autplot() plots showing time-varying coefficients for the tourism model, illustrating how basis coefficients have been estimated to vary through time." width="100%" />
...
automatic plotting and/or scoring of forecasts
    ...
    #> 1 2016 Q4 Melbourne Victor… Visiti…  804.       4    76 sample[200]  850. FFC_E…
    #> 2 2017 Q1 Melbourne Victor… Visiti…  734.       1    77 sample[200]  762. FFC_E…
    #> 3 2017 Q2 Melbourne Victor… Visiti…  670.       2    78 sample[200]  773. FFC_E…
    #> 4 2017 Q3 Melbourne Victor… Visiti…  824.       3    79 sample[200]  741. FFC_E…
    #> 5 2017 Q4 Melbourne Victor… Visiti…  985.       4    80 sample[200]  840. FFC_E…
...
Leverage the fabletools ecosystem for forecast analysis
    ...
    #> 1 FFC_ETS Melbo… Vict… Visiti… Test   10.1  91.0  80.9 -0.0453  9.94   NaN   NaN
    ...
    #>     <qtr>         <dbl>           <dbl> <dbl> <dbl>
    #> 1 2016 Q4          850.            850.  795.  897.
    #> 2 2017 Q1          762.            759.  708.  810.
    #> 3 2017 Q2          773.            768.  730.  818.
    #> 4 2017 Q3          741.            738.  689.  789.
    #> 5 2017 Q4          840.            842.  773.  907.
