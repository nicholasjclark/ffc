...
test_that("predict.ffc_gam() works with se.fit = TRUE", {
  ...
  pred_se_link <- predict(
    example_mod,
    newdata = test_newdata,
    type = "link",
    se.fit = TRUE
  )
  pred_se_response <- predict(
    example_mod,
    newdata = test_newdata,
    type = "response",
    se.fit = TRUE
...
test_that("predict.ffc_gam() preserves prediction consistency", {
  ...
  pred_reordered <- predict(
    example_mod,
    newdata = reordered_newdata,
    type = "link"
  )
...
test_that("predict.ffc_gam() handles lpmatrix correctly", {
  ...
  manual_pred <- as.vector(
    pred_lpmatrix %*% coef(example_mod) + attr(pred_lpmatrix, "model.offset")
  )
  auto_pred <- as.vector(predict(
    example_mod,
    newdata = test_newdata,
    type = "link"
  ))
...
test_that("distributional families handle invalid link function calls", {
  ...
  result <- SW(ffc:::apply_distributional_inverse_links(
    invalid_predictions,
    gaulss_fam
  ))
  ...
  empty_result <- ffc:::apply_distributional_inverse_links(
    empty_predictions,
    gaulss_fam
  )
