...
create_forecast_newdata <- function(
  base_time,
  n_ahead = 5,
  groups = letters[1:3],
  seed = 456
...
test_that("forecasts identical regardless of tsibble ordering", {
  ...
  # Generate forecasts
  SW({
    forecast_ordered <- forecast(
      model_ordered,
      newdata = newdata_tsibble,
      model = "ETS"
    )
    forecast_shuffled_data <- forecast(
      model_shuffled,
      newdata = newdata_tsibble,
      model = "ETS"
    )
    forecast_shuffled_newdata <- forecast(
      model_ordered,
      newdata = newdata_shuffled,
      model = "ETS"
    )
  })
  ...
  expect_equal(
    forecast_ordered$.estimate,
    forecast_shuffled_data$.estimate,
    tolerance = 0.1
  )
  ...
  fc_shuf_sorted <- forecast_shuffled_newdata[
    order(seq_len(nrow(forecast_shuffled_newdata))),
  ]

  expect_equal(
    fc_ord_sorted$.estimate,
    fc_shuf_sorted$.estimate,
    tolerance = 0.1
  )
})
...
test_that("forecasts work with mixed data.frame and tsibble inputs", {
  ...
  SW({
    forecast_df_tsibble <- forecast(
      model_df,
      newdata = pred_tsibble,
      model = "ETS"
    )
    forecast_tsibble_df <- forecast(
      model_tsibble,
      newdata = pred_df,
      model = "ETS"
    )
    forecast_df_df <- forecast(model_df, newdata = pred_df, model = "ETS")
    forecast_tsibble_tsibble <- forecast(
      model_tsibble,
      newdata = pred_tsibble,
      model = "ETS"
    )
  ...
  expect_equal(
    forecast_df_tsibble$.estimate,
    forecast_tsibble_df$.estimate,
    tolerance = 0.1
  )

  expect_equal(
    forecast_df_df$.estimate,
    forecast_tsibble_tsibble$.estimate,
    tolerance = 0.1
  )
})
...
test_that("intermediate forecasting steps maintain row correspondence", {
  ...
  SW({
    model1 <- ffc_gam(y ~ fts(x, k = 3), data = ordered_data, time = "time")
    model2 <- ffc_gam(
      y ~ fts(x, k = 3),
      data = reverse_ordered_data,
      time = "time"
    )
  })
...
test_that("systematic ordering patterns handled robustly with grouped data", {
  ...
  SW({
    model_normal <- ffc_gam(
      y ~ fts(x, by = group, k = 3),
      data = normal_order,
      time = "time"
    )
    model_reverse <- ffc_gam(
      y ~ fts(x, by = group, k = 3),
      data = reverse_time,
      time = "time"
    )
    model_interleaved <- ffc_gam(
      y ~ fts(x, by = group, k = 3),
      data = interleaved,
      time = "time"
    )
    model_scrambled <- ffc_gam(
      y ~ fts(x, by = group, k = 3),
      data = group_scrambled,
      time = "time"
    )
  })
...
test_that("complex hierarchical grouping structures maintain ordering robustness", {
  ...
  set.seed(666)
  hierarchical_data <- create_grouped_data(
    n_groups = 3,
    n_times = 20,
    seed = 666,
  ...
  hierarchical_data <- hierarchical_data |>
    dplyr::mutate(
      treatment = factor(rep(
        c("A", "B"),
        length.out = nrow(hierarchical_data)
      )),
      site = factor(rep(
        c("Site1", "Site2"),
        length.out = nrow(hierarchical_data)
  ...
  newdata <- data.frame(
    ...
    treatment = factor(
      rep("A", 5),
      levels = levels(hierarchical_data$treatment)
    ),
