...
1. Read the correct TRD and task list docs in `tasks/`. Check which sub-task is next
2. Implement sub-task following R package best practices
3. **Code Review:** Use `code-reviewer` agent for ALL proposed R code changes BEFORE making changes
4. Address ALL PRIORITY items from the `code-reviewer` immediately
5. Write comprehensive tests in `tests/testthat/`
6. **STOP AND WAIT:** Update task list, mark sub-task `[x]`, and ask user "Ready for the next sub-task?" - Wait for "yes" or "y" before proceeding
